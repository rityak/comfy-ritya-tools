...
export function WeightSumConfig() {
  ...
  // State for global weights string
  const [globalWeights, setGlobalWeights] = useState('0.5,0.5')
  // State for showing all blocks vs global only
  const [showAllBlocks, setShowAllBlocks] = useState(false)
  // State for block weights
  const [blockWeights, setBlockWeights] = useState<Record<string, string>>({})
  ...
  const generateJson = () => {
    ...
    if (showAllBlocks) {
      ...
      const specificBlocks: Array<{ name: string; weight: string }> = []

      blockGroups.forEach((group) => {
        group.subBlocks.forEach((subBlock) => {
          // Check if specific block is enabled and has different weights than its group
          const groupWeight = blockWeights[group.name] || globalWeights
          if (
            enabledBlocks[subBlock] &&
            blockWeights[subBlock] !== groupWeight &&
            blockWeights[subBlock] !== globalWeights
          ) {
            specificBlocks.push({
              name: subBlock,
      ...
      const groupBlocks: Array<{ name: string; weight: string }> = []

      blockGroups.forEach((group) => {
        if (
          enabledBlocks[group.name] &&
          blockWeights[group.name] !== globalWeights
        ) {
          groupBlocks.push({
            name: group.name,
  ...
  const updateBlockWeights = (blockName: string, weights: string) => {
    setBlockWeights((prev: Record<string, string>) => ({
      ...prev,
      [blockName]: weights
    }))
  }
  ...
  const normalizeBlockWeights = (blockName: string) => {
    const weightsStr = blockWeights[blockName] || globalWeights
  ...
  const resetToDefaults = () => {
    const newWeights = Array(numModels).fill(1 / numModels)
    setModelWeights(newWeights)
    const weightsStr = newWeights.map((w) => w.toFixed(3)).join(',')
    setGlobalWeights(weightsStr)
            ...
            <div key={group.name} className="block-group">
              <div className="group-header">
                ...
                <div className="group-input-row">
                  <input
                    type="text"
                    value={blockWeights[group.name] || globalWeights}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  const renderHeatmap = () => {
    ...
    blockGroups.forEach((group) => {
      if (group.subBlocks.length > 0) {
        group.subBlocks.forEach((subBlock) => {
          if (enabledBlocks[subBlock]) {
            const subBlockWeightStr = blockWeights[subBlock] || globalWeights
      ...
      if (enabledBlocks[group.name]) {
        const groupWeightStr = blockWeights[group.name] || globalWeights
        const groupWeights = groupWeightStr
          ? groupWeightStr.split(',').map((w: string) => parseFloat(w) || 0)
          : Array(numModels).fill(0)
        heatmapData.push({
          name: group.name,
  ...
  return (
    <div className="weight-sum-config">
      <div className="config-header">
        <h2>Weight Sum Configuration</h2>
        <p>Create JSON configuration for weight_sum_with_json function</p>
      </div>
