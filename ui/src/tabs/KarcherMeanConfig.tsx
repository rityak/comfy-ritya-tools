...
export function KarcherMeanConfig() {
  ...
  useEffect(() => {
    ...
    setConfig((prev: typeof config) => ({
      ...prev,
      global: newWeights.map((w: number) => w.toFixed(3)).join(',')
    }))
  ...
  useEffect(() => {
    setConfig((prev: typeof config) => ({
      ...prev,
      global: modelWeights.map((w: number) => w.toFixed(3)).join(',')
    }))
  }, [modelWeights])
  ...
  const renderHeatmap = () => {
    ...
    if (heatmapData.length === 0) {
      const globalWeights = config.global
        ? config.global.split(',').map((w: string) => parseFloat(w) || 0)
        : Array(numModels).fill(0)
      heatmapData.push({
        name: 'global',
  ...
  return (
    <div className="karcher-config">
      <div className="config-header">
        <h2>Karcher Mean Configuration</h2>
        <p>Create JSON configuration for karcher_mean_with_json function</p>
      </div>
